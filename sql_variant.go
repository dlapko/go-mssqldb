...
func (v *SQLVariant) encode() ([]byte, error) {
	...
	case typeDateTim4:
		return v.encodeDateTim4(time.UTC)
	case typeFlt4:
...
func (v *SQLVariant) encodeDateTim4(loc *time.Location) ([]byte, error) {
	var t time.Time
	switch v := v.Value.(type) {
	case time.Time:
		t = v
	...
	valueBytes := encodeDateTim4(t, loc)
	return v.encodeZeroProp(valueBytes), nil
}
...
func (v *NullSQLVariant) decodeVariant(buf []byte) {
	...
	case typeDateTime:
		sqlVariant.Value = decodeDateTime(r.readValue(valueSize), time.UTC)
	case typeDateTim4:
		sqlVariant.Value = decodeDateTim4(r.readValue(valueSize), time.UTC)
	case typeFlt4:
	...
	case typeDateN:
		sqlVariant.Value = decodeDate(r.readValue(valueSize), time.UTC)
	case typeTimeN:
		scale := r.byte()
		sqlVariant.Scale = scale
		sqlVariant.Value = decodeTime(scale, r.readValue(valueSize), time.UTC)
	case typeDateTime2N:
		scale := r.byte()
		sqlVariant.Scale = scale
		sqlVariant.Value = decodeDateTime2(scale, r.readValue(valueSize), time.UTC)
	case typeDateTimeOffsetN:
		scale := r.byte()
		sqlVariant.Scale = scale
