...
func readVariantTypeRaw(ti *typeInfo, r *tdsBuffer, c *cryptoMetadata) interface{} {
	size := r.int32()
	if size == 0 {
		return nil
	}
...
func writeVariantType(w io.Writer, ti typeInfo, buf []byte) (err error) {
	if buf == nil {
	...
	case typeText, typeImage, typeNText, typeVariant:
		...
		case typeVariant:
			if encoding.RawSqlVariant {
				ti.Buffer = make([]byte, ti.Size)
				ti.Reader = readVariantTypeRaw
			} else if encoding.GuidConversion {
